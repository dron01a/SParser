...
XML::XML(std::string xml_file){
    source = new file(xml_file);
}
...
std::string XML::get_tag_value(std::string src){
    std::string result;
    if(src[src.length() - 1] != '/'){
...
attribTAG XML::get_tag_attrib(std::string src){
    attribTAG result;
    ...
    if(*prop_len > 0){
        ...
        string_content * tag_prop = new string_content(*prop_str, " \"\'=");
        if(tag_prop->size() != 1 ){
...
std::string XML::get_tag_name(std::string src){
    string_content tag_prop(src, " \t<>\"\'=");
    return tag_prop[0];
}

TAG XML::get_sub_tag(int & start_pos){
    TAG result;
    std::string *temp = new std::string(source->get(start_pos));
    std::remove(temp->begin(), temp->end(), '\t'); 
    if(!temp->empty()){
        result.attrib = get_tag_attrib(*temp);
        result.name = get_tag_name(*temp);
        result.value = get_tag_value(*temp);
...
int XML::get_tag_position(std::string tag_name, std::string prop, std::string val){
    int result = source->find(tag_name);
    std::string * temp = new std::string;
    *temp = source->get(result);
    string_content * str;
    str = new string_content(*temp," \t\"\'=");
...
TAG XML::getTAG(std::string tag_name, std::string prop, std::string val){
    TAG result;
...
TAGArray XML::select(std::string tagName){
    TAGArray result;
    ...
    while(*tagPos != -1){
        ...
        tag.attrib = get_tag_attrib(*temp);
        tag.name = get_tag_name(*temp);
        tag.value = get_tag_value(*temp);
